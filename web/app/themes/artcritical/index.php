...
<?php if (have_posts())  : ?>
	<?php while (have_posts()) : the_post(); ?>
		...
		<?php ?>
		<?php $coverdescription = get_fronttop3_excerpt(55);?>
...
<?php if (have_posts() && $wp_query->post_count > 2)  : ?>
	...
	<div id="threefeatured">
		<div id="images">
		<?php while (have_posts()) : the_post(); ?>
			<div class="image" id="feature_image_<?php echo $count?>" style="background:url(<?php echo postimage('featured_front'); ?>) center center; <?= $count == 1 ? '':'display:none'?> "></div>
			<?php
			$count++;
			?>
		<?php endwhile;?>
		...
		<?php while (have_posts()) : the_post(); ?>
			<div class="theexcerpt <?= $count == 1 ? 'selected':''?>" id="feature_excerpt_<?php echo $count?>" onmouseover="feature_tab_quick(<?php echo $count?>, '<?php the_permalink(); ?>');">
				<div class="title_excerpt">
					...
					<div class="excerpt">
						<?php echo get_fronttop3_excerpt(); ?>
					</div>
				</div>
...
<?php endif;?>
<div id="columns">
	<?php get_sidebar('column1'); ?>

	<?php get_sidebar('column2'); ?>

	<?php get_sidebar('column3'); ?>
	<br style="clear:both">
</div>
